...
| *"any other host is refused by code"* (LOCAL_TIER, non-loopback) | `modules/02-enforcement/hook_prompt_class.py` (`_is_loopback_host_port`, `tier2_classify`) | `hook_prompt_class.py --selftest`, check (c): a non-loopback host is refused before any call, asserted against `CALL_ATTEMPTS` | **CHECKED** |
