...
## Round: EYES 1 FIX — 2026-09-16 (source: the maintainer's round-one spec-side review, finding 3, a private record)

| Ruling (verbatim where possible) | Landed in | Enforcing check | Status |
|---|---|---|---|
| Lineage (the eyes design note, section 5, a private record, ship requirements): nearest published ancestors are Chromium headless screenshot (the render mechanism) and pixel-diff visual regression (the gate shape — compare against a stored render, fail on change); what is new is the ten-question catalog as the judged content, answered by a reader rather than a pixel diff, with mechanical exists/fresh/dimension/blank checks running first and needing no model. | `modules/03-verification/eyes.py`, `EYES.md`, opening its "Lineage." paragraph | `checks-registry.json` → `selftest:eyes`, `gate:eyes` (seen_red 2026-09-16) | **CHECKED** |
| ORACLE-DECLINED: a reader's actual visual judgment of the fixture is never machine-verified — `--selftest` section H now judges a JSON-shaped answers file against `FIXTURE_EXPECTED_YES` (`judge_fixture_reading`), which proves the judging *shape* is real, but nothing in CI reads the fixture PNG itself and forms an opinion; the check's shape would be a scripted vision-model call inside `--selftest` that answers the catalog against `examples/eyes-fixture/before-1280.png` and asserts the result matches `FIXTURE_EXPECTED_YES`. | round EYES-1-FIX | not built: `eyes.py` is stock Python, no dependency, no live model call, by design (module docstring; decision 5's network-blocked-by-default render); a vision call inside `--selftest` would need network access and a model credential, breaking the "no browser, no filesystem" selftest promise the rest of the suite holds to; trigger: a level-2 harness with its own model-calling budget (held, per the eyes design note, a private record section 3). | **ORACLE-DECLINED** |

---

## Round: PROMPT CLASS KIT — 2026-09-16 (source: `protocols/PROMPT-CLASS-KIT-charter-20260916.md`, a private record, and its 15:5x amendment)

| Ruling (verbatim where possible) | Landed in | Enforcing check | Status |
|---|---|---|---|
| *"the class lists load from prompt_class.template.json and then from PROMPT_CLASS_FIXTURES (a .local path, later wins), never from constants that carry an owner's words"* | `modules/02-enforcement/hook_prompt_class.py` (`load_class_data`, `_merge_layer`), `modules/02-enforcement/prompt_class.template.json` | `hook_prompt_class.py --selftest`, check (a): every template fixture labels correctly against the loaded (not hard-coded) class data | **CHECKED** |
| *"any other host is refused by code"* (LOCAL_TIER, non-loopback) | `modules/02-enforcement/hook_prompt_class.py` (`_is_loopback_host_port`, `_is_loopback_url`, `tier2_classify`) | `hook_prompt_class.py --selftest`, check (c): widened past one plain foreign host to the userinfo form, a non-loopback bracketed IPv6 literal, a trailing-dot host and a scheme-prefixed value, each validated against `urlsplit(url).hostname` rather than a first-colon split, each asserted against `CALL_ATTEMPTS` (seen_red 2026-09-16: the round 1 review's own failing input, `localhost:11434@<foreign-host>`, was the first red — the form the prior version of check (c) never exercised) | **CHECKED** |
| *"LOCAL_TIER = NONE means tier 1 only, the local call never attempted"* | `modules/02-enforcement/hook_prompt_class.py` (`classify`, `LOCAL_ENABLED`) | `hook_prompt_class.py --selftest`, check (d): `LOCAL_TIER` NONE never attempts a call, asserted against `CALL_ATTEMPTS` | **CHECKED** |
| ORACLE-DECLINED: whether `hook_fixtures.py --armed` (or an equivalent) proves `hook_prompt_class.py` is actually wired into a live, adopted `.claude/settings.json` — `check_armed()` reads only the `PreToolUse` block by its own scope, and this round did not extend it to `UserPromptSubmit`. The check's shape would be a `--armed`-style parse of the `UserPromptSubmit` block asserting the hook's command string is present and its script resolves and exists. | round PROMPT-CLASS-KIT | not built: extending `hook_fixtures.py`'s claim-1 scope to a second hook event was outside this round's touch set (`kit.config`, `settings.json.template`, `hook_prompt_class.py`, its template, `.gitignore`, one README section, one ledger row — no touch to `hook_fixtures.py` itself); trigger: a charter naming `hook_fixtures.py` as a touch and asking for a `UserPromptSubmit`-scoped arming claim. | **ORACLE-DECLINED** |
