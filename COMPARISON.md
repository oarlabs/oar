...
| C4 | Forced red — a negative-control facility, a dead-man clause, and a per-check record of the last observed refusal (`BLUEPRINT.md` §5, `DECISION-BRIEF.md`). **Narrowed in round 32:** the row previously claimed the property over *every* check in the kit, which nothing in the kit could substantiate for any single one. The registry's `seen_red` field is the instrument; at 0.1.0 it is 28 dated rows out of 201 | PARTIAL-OVERLAP on the mechanism; NO-MATCH-FOUND as a shipped governance gate | The mechanism is 48 years old: mutation testing, DeMillo/Lipton/Sayward 1978 [SEARCH-URL: `https://www.scirp.org/reference/referencespapers?referenceid=953139`]. **Agentic OS** ships one negative control — a planted credential that must block the commit [FETCHED, as above]. Not found anywhere: a governance kit requiring *every* check to carry a recorded red proof, with a dead-man clause scoring a silent gate as zero rather than partial |
...
| C13 | The collaboration layer: evidenced owner defaults, a five-question seed interview, and a living owner profile the AI maintains (module 08) | **PARTIAL-OVERLAP**, narrowed to the governance properties | **RE-RULED IN ROUND 32, on the C11 precedent, from an adversarial prior-art lane.** The row previously read NO-MATCH-FOUND and called module 08 "the kit's most differentiated module", resting on the sentence "nothing in the agent-governance landscape surveyed models the **human** as a cached artifact". That qualifier was doing the work: a document whose declared subject is how a named person works with others is an established practitioner genre outside the surveyed landscape and older than it. **Manual of Me** is a shipped service that defines its output as "a document which helps others understand how they can work best with you" and invites visitors to "join over 30,000 people who have created theirs" [FETCHED: `https://www.manualof.me/`]; the agile **team working agreement** records decisions about how parties work together [SEARCH-URL: `https://www.easyagile.com/blog/team-charter-working-agreement-social-contract-template-guide`]; and user modelling is a named research subject from at least Rich, "User Modeling via Stereotypes", Cognitive Science 3(4) 329–354, 1979 [FETCHED, bibliographic record only: `https://dblp.org/rec/journals/cogsci/Rich79.html`. The paper was not opened; this row cites the existence of the subject, not any result in it]. **Agentic OS** carries decisions across sessions in a state file [FETCHED, as above] but models the work, not the owner. What survives is the **governance**, not the subject: none of the genre artifacts above is append-only, superseded in place, owner-gated with a proposal path, or verbatim-provenanced. `modules/08-collaboration/CAPSULE.md` states the same restatement in its own words. Weakest-evidenced by its own admission either way: n = 1 owner |
...
**The tally, recounted for this page rather than carried over.** Thirteen rows
carry a PARTIAL-OVERLAP verdict — nine wholly (C1, C2, C6, C8, C9, C10, C13,
C15, C16) and four in part, where a sub-claim is unmatched (C3, C4, C5, C7).
Two
are wholly NO-MATCH-FOUND (C14, C17) and one is NO-MATCH-FOUND on a
sub-claim only (C11, narrowed in round 30 by a reader's counterexample). One
is REDUNDANT-BY (C12). One is the composition claim itself (C18). C13 moved
from NO-MATCH-FOUND to PARTIAL-OVERLAP in round 32; the tally is recounted
here rather than adjusted, which is why the round-30 and round-32 movements
