...
| 7 — the ledgers (L1 step 3) | MIXED | Copying, substituting and the first `FAILURE-FLOOR.md` row are MECHANICAL — that row is the model-tiering hook, layer `HOOK`, zone `B`, status `STRUCTURAL`, last fired `never` (`QUICKSTART.md` Step 7, item 2). A ledger-name collision ruling and each seed lesson are OWNER JUDGMENT (D-7). `RATIO_CEILING` stays as shipped: SHIPPED DEFAULT, the one placeholder allowed to survive |
...
**`KNOWLEDGE_DIR`.** `LEVEL-1.md`:42 calls it one of two things that "are
...
| 4 | The host is genuinely empty: one commit, no source, no `CLAUDE.md`, no `.claude/settings.json` — nothing to read agent vocabulary from either way | Do not infer case 1 from the absence of evidence. Treat it as case 2: OWNER JUDGMENT, and the four keys block until answered. Record `DEFERRED` in §7 and say the host was empty |
