...
# rules template renders to 260 lines (296 in the file, minus the 36-line
...
# itself. The shipped binding digest is therefore 260 + 90 = 350 lines.
# STEP 2 - 350 x 1.15 = 402.50, rounded up to the nearest 25: 425.
# RE-DERIVED SIX TIMES NOW - TWICE IN ROUND 24, AGAIN IN ROUND 25, AGAIN IN
# ROUND 26, AGAIN IN ROUND 32, AGAIN AT THE 0.1.8 HOTFIX - WHICH IS THE
# BINDING WORKING rather than a number being chased.
...
#   0.1.8     the refusal ledger's wrapper rule and REFUSALS section grew the
#             template 251 -> 260; the guard fired in CI on the 0.1.7 tag (both
#             runners, `got 260, want 251`) because the release ran without the
#             doctor's selftest; ceiling re-derived 400 -> 425.
# Measured, not assumed: run with a stale constant the selftest fails naming
# both numbers (`got 260, want 251` the last time), which is exactly what the
...
# the other is arithmetic. The half that carries no information: at 425 the
# kit's own shipped pair passes, which is true by construction because step 2
# set the ceiling above it. The half that does: the shipped pair sits 75 lines
# (17.6%) below the line rather than at it, so ordinary project-specific
# additions fit, while a rules file that has merely DOUBLED from the shipped
# template (520 lines) breaches it on its own with no checkpoint at all. That
...
DIGEST_SHIPPED_RULES_LINES = 260
DIGEST_CHECKPOINT_NORM_LINES = 90
DIGEST_CEILING_LINES = 425
